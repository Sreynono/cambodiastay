  ...
  <div class="min-h-screen bg-[#F8F7F2] text-[#2C3E50] flex flex-col justify-between">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10 w-full flex-1">
        ...
        <div class="bg-white rounded-3xl sm:rounded-full p-2 sm:p-2.5 shadow-md border border-gray-200 flex flex-col sm:flex-row items-stretch sm:items-center divide-y sm:divide-y-0 sm:divide-x divide-gray-100 sm:divide-gray-200 w-full max-w-3xl">
          ...
          <div class="flex-1 px-4 sm:px-6 py-2 sm:py-1">
            ...
            <input
              ...
              type="text"
              placeholder="e.g. Mountain, Rice Farm, Kampot, River..."
          ...
          <div class="flex-1 px-4 sm:px-6 py-2 sm:py-1 hidden sm:block">
            <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Dates</label>
            <input
          ...
          <div class="flex-1 px-4 sm:pl-6 sm:pr-2 py-2 sm:py-1 flex items-center justify-between">
            <div class="min-w-0 flex-grow">
              <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Guests</label>
              <input
          ...
          <h1 class="text-3xl md:text-4xl font-serif font-bold text-[#113A28]">
            {{
              searchQuery.location
                ? `Stays near "${searchQuery.location}"`
                : selectedCategory === 'All'
                  ? 'Heritage Homestays'
                  : selectedCategory + ' Homestays'
            }}
          </h1>
          <p class="text-sm text-gray-600 mt-1">
            Discover {{ filteredHomestays.length }} verified stays across Cambodia's countryside landscapes.
          </p>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
          ...
          <div class="lg:w-2/3 bg-[#EAE7DC]/60 rounded-2xl p-6 relative min-h-[420px] flex flex-col justify-between border border-gray-200 overflow-hidden">
            <div class="flex justify-between items-start z-10">
              <div>
                <h3 class="font-bold text-sm text-[#113A28] uppercase tracking-wider">Interactive Cambodia Map</h3>
                <p class="text-xs text-gray-600">Select a province hotspot or landmark to filter homestays</p>
              </div>
              ...
              >
                <div class="flex flex-col items-center">
                  ...
                  <span class="text-[10px] bg-white/90 px-1.5 py-0.5 rounded shadow-sm text-gray-600 font-semibold mt-1">
                    {{ getStaysCountForProvince(pin.name) }} stays
                  </span>
                </div>
              </button>
            </div>
            ...
            <div class="flex justify-between items-center text-xs text-gray-500 z-10">
              <span>Tip: Click any marker to filter properties by place.</span>
          ...
          <div class="lg:w-1/3 flex flex-col justify-between">
            <h3 class="font-bold text-base text-gray-900 mb-4">
              {{ searchQuery.location ? `Stays in or near ${searchQuery.location}` : 'All Featured Places' }}
            </h3>
            ...
            <div class="space-y-4 max-h-[380px] overflow-y-auto pr-2">
              ...
              >
                ...
                />
                <div v-else class="w-20 h-20 rounded-xl bg-gradient-to-br from-[#113A28] to-emerald-800 flex items-center justify-center text-white text-2xl">
                  🏡
                </div>
                <div class="flex-1">
                  <h4 class="font-bold text-sm text-gray-900 line-clamp-1">{{ prop.name }}</h4>
                  <p class="text-xs text-gray-500 mb-1">📍 {{ prop.province }} · {{ prop.landscape || prop.category }}</p>
                  <div class="flex justify-between items-center">
                    <span class="text-xs font-bold text-[#113A28]">${{ prop.price }} / night</span>
        ...
        >
          ...
          <div class="relative h-56 w-full bg-gray-100 overflow-hidden">
            ...
            <div v-else class="w-full h-full bg-gradient-to-br from-[#113A28] to-emerald-800 flex flex-col items-center justify-center text-white p-4">
              ...
              <span class="text-[10px] text-emerald-300 uppercase tracking-widest mt-1">{{ prop.province }}</span>
            </div>
            ...
            <button
              ...
              class="absolute top-4 right-4 p-2 bg-white/90 hover:bg-white rounded-full shadow-sm text-sm transition"
              title="Save to Wishlist"
            >
              <span v-if="propertyStore.isWishlisted(prop.id)">❤️</span>
          ...
          <div class="p-6">
            <div class="flex justify-between items-start mb-1">
              ...
              <span class="text-sm font-bold flex items-center gap-1 text-gray-800">
                <span class="text-yellow-500">★</span> {{ prop.rating || '4.9' }}
              </span>
            </div>

            <p class="text-xs text-gray-500 mb-2 capitalize">📍 {{ prop.province ? prop.province + ', Cambodia' : 'Cambodia' }}</p>
      ...
      <div v-else class="text-center py-20 bg-white rounded-3xl border border-gray-200 shadow-sm p-8">
        ...
        <h3 class="text-xl font-bold text-gray-800 mb-2">No Approved Homestays Found</h3>
        <p class="text-gray-500 text-sm max-w-md mx-auto mb-6">
          No live homestays match your place search "{{ searchQuery.location || selectedCategory }}". Try searching for places like <strong>Mountain</strong>, <strong>Rice Farm</strong>, <strong>Kampot</strong>, <strong>Riverside</strong>, or <strong>Siem Reap</strong>.
        </p>
